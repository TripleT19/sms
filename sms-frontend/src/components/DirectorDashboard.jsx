...
const DirectorDashboard = () => {
  ...
  return (
    <div className="p-6 bg-blue-50 min-h-screen">
      <h1 className="text-3xl font-bold text-blue-900 mb-8">Director's Dashboard</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div className="bg-white p-6 rounded-xl shadow flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-2xl font-bold text-amber-900">{performance.classes.length > 0 ? (performance.classes.reduce((sum, c) => sum + (c.average || 0), 0) / performance.classes.length).toFixed(1) + '%' : '—'}</p>
          </div>
        </div>
      </div>

      {/* Performance by Class & Finance Summary */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        ...
        <div className="bg-white rounded-xl shadow p-6">
          ...
          <div className="flex justify-between mb-4">
            <span>Total Billed</span><span className="font-bold">MK {finance.total_fees?.toLocaleString()}</span>
          </div>
          <div className="flex justify-between mb-4">
            <span>Collected</span><span className="font-bold text-green-600">MK {finance.collected?.toLocaleString()}</span>
          </div>
          <div className="flex justify-between">
            <span>Outstanding</span><span className="font-bold text-red-600">MK {finance.outstanding?.toLocaleString()}</span>
          </div>
        </div>
      </div>

      {/* Tasks, Announcements, Logs */}
            ...
            <ul className="space-y-2">
              {tasks.slice(0, 5).map(task => (
                <li key={task.id} className="p-2 bg-gray-50 rounded">
                  ...
                  <p className="text-xs text-gray-500">{task.deadline ? 'Due: ' + task.deadline : ''}</p>
                </li>
              ))}
            </ul>
          )}
            ...
            <ul className="space-y-2">
              {announcements.slice(0, 5).map(ann => (
                <li key={ann.id} className="p-2 bg-gray-50 rounded">
                  ...
                  <p className="text-xs text-gray-500">{ann.created_at}</p>
                </li>
              ))}
            </ul>
          )}
