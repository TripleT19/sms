...
const FinancePage = () => {
  ...
  const fetchFeeTypes = async () => {
    const res = await fetch(`${API_BASE}/api/fees/fee-types`, {
      headers: { Authorization: `Bearer ${token}`, Accept: 'application/json' },
    });
    if (res.ok) setFeeTypes(await res.json());
  };

  const fetchBankDetails = async () => {
    const res = await fetch(`${API_BASE}/api/fees/bank-details`, {
      headers: { Authorization: `Bearer ${token}`, Accept: 'application/json' },
    });
    if (res.ok) setBankDetails(await res.json());
  };

  useEffect(() => { fetchFeeTypes(); fetchBankDetails(); }, []);
  ...
  // ==================== ENSURE MANDATORY FEES & FETCH ====================
  useEffect(() => {
    if (selectedClass && selectedTerm) {
      fetch(`${API_BASE}/api/fees/ensure-mandatory`, {
        method: 'POST',
        ...
        .catch(() => showModal('error', 'Failed to ensure mandatory fees'));
    } else {
      setStudents([]);
      setStudentFees([]);
    }
  ...
  const fetchStudentsAndFees = async () => {
    // Fetch students
    const params = new URLSearchParams({ class_id: selectedClass, stream_id: selectedStream || '' });
    const studentsRes = await fetch(`${API_BASE}/api/fees/students-by-class?${params}`, {
      headers: { Authorization: `Bearer ${token}`, Accept: 'application/json' },
    });
    ...
    const feesRes = await fetch(`${API_BASE}/api/fees/student-fees?${feesParams}`, {
      headers: { Authorization: `Bearer ${token}`, Accept: 'application/json' },
    });
    if (feesRes.ok) setStudentFees(await feesRes.json());
  ...
  const addFeeRow = () => {
    setAssignFeesList([...assignFeesList, { fee_type_id: '', amount: '', location: '' }]);
  };

  const removeFeeRow = (index) => {
    setAssignFeesList(assignFeesList.filter((_, i) => i !== index));
  };
  ...
  const handleAssignMultiSubmit = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/fees/assign-fees`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
  ...
  // ==================== EDIT PAID AMOUNT ====================
  const openEditPaidModal = (fee) => {
    setEditingFeeForPaid(fee);
    setNewPaidAmount(fee.paid_amount);
    setEditPaidModal(true);
  };

  const handleEditPaidAmount = async () => {
    if (newPaidAmount === '' || isNaN(newPaidAmount) || Number(newPaidAmount) < 0) {
      showModal('error', 'Please enter a valid paid amount');
      return;
    }
    setSavingPaidAmount(true);
    try {
      const res = await fetch(`${API_BASE}/api/fees/student-fee/${editingFeeForPaid.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
  ...
  const handleDeleteFee = (fee) => {
    confirmAction(
      ...
      `Are you sure you want to remove "${fee.fee_type?.name}"?`,
      async () => {
        const res = await fetch(`${API_BASE}/api/fees/student-fee/${fee.id}`, {
          method: 'DELETE',
  ...
  const handleRecordPayment = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const err = await res.json();
        if (err.overpayment) {
          showModal('error', `Overpayment of MK ${err.overpayment}. Please use the transfer function.`);
        } else {
          showModal('error', err.message || 'Failed');
        }
      }
  ...
  const openTransferModal = (sourceFee) => {
    setTransferSource(sourceFee);
    setTransferTarget('');
    setTransferAmount('');
  ...
  const handleTransfer = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/fees/transfer-overpayment`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
        body: JSON.stringify({
          from_student_fee_id: transferSource.id,
          to_student_fee_id: transferTarget,
  ...
  const downloadInvoice = async (feeId) => {
    ...
    } catch (err) { showModal('error', 'Network error'); }
  };
  ...
  const openFeeTypeModal = (ft = null) => {
    if (ft) {
      ...
      setFeeTypeForm({
        ...
        class_id: ft.class_id || '',
        stream_id: ft.stream_id || '',
  ...
  const addAmountRow = () => {
    setFeeTypeForm({
      ...feeTypeForm,
      amounts: [...feeTypeForm.amounts, { class_id: '', stream_id: '', location: '', amount: '' }],
    });
  };

  const removeAmountRow = (index) => {
    setFeeTypeForm({ ...feeTypeForm, amounts: feeTypeForm.amounts.filter((_, i) => i !== index) });
  };
  ...
  const handleDeleteFeeType = (id, name) => {
    confirmAction(
      ...
      async () => {
        await fetch(`${API_BASE}/api/fees/fee-types/${id}`, { method: 'DELETE', headers: { Authorization: `Bearer ${token}` } });
        fetchFeeTypes();
  ...
  const handleBankSubmit = async (e) => {
    ...
    const url = editingBank
      ? `${API_BASE}/api/fees/bank-details/${editingBank.id}`
      : `${API_BASE}/api/fees/bank-details`;
  ...
  const handleDeleteBank = (id) => {
    confirmAction('Delete Bank Detail', 'Are you sure?', async () => {
      await fetch(`${API_BASE}/api/fees/bank-details/${id}`, { method: 'DELETE', headers: { Authorization: `Bearer ${token}` } });
      fetchBankDetails();
  ...
  return (
    <div className="p-4 md:p-6 bg-blue-50 min-h-screen">
      ...
      <div className="flex gap-4 mb-6">
        <button
          onClick={() => setActiveTab('feesAndPayments')}
          className={`px-4 py-2 rounded-full font-medium transition ${activeTab === 'feesAndPayments' ? 'bg-blue-600 text-white' : 'bg-white text-blue-600 border border-blue-600 hover:bg-blue-50'}`}
        >
          <FaMoneyBillWave className="inline mr-2" /> Fees & Payments
        </button>
        <button
          onClick={() => setActiveTab('feeTypes')}
          className={`px-4 py-2 rounded-full font-medium transition ${activeTab === 'feeTypes' ? 'bg-blue-600 text-white' : 'bg-white text-blue-600 border border-blue-600 hover:bg-blue-50'}`}
        >
          <FaFileInvoice className="inline mr-2" /> Fee Types
        </button>
        <button
          onClick={() => setActiveTab('banking')}
          className={`px-4 py-2 rounded-full font-medium transition ${activeTab === 'banking' ? 'bg-blue-600 text-white' : 'bg-white text-blue-600 border border-blue-600 hover:bg-blue-50'}`}
        >
          <FaUniversity className="inline mr-2" /> Banking
        </button>
      </div>
                  ...
                  <table className="w-full text-left">
                    <thead className="bg-blue-50">
                      <tr>
                        ...
                        <th className="p-4">Fees Summary</th>
              ...
              <tbody>
                {feeTypes.map(ft => (
                  <tr key={ft.id} className="border-t">
                    ...
                    <td className="p-4">{ft.category}</td>
                    <td className="p-4">
                      {ft.amounts.map((amt, i) => (
                        <div key={i} className="text-sm">
                          {amt.class ? `${amt.class.name}${amt.stream ? ` ${amt.stream.name}` : ''}: ` : ''}
                          {amt.location ? `${amt.location}: ` : ''}
                          MK {Number(amt.amount).toLocaleString()}
                        </div>
                      ))}
                    </td>
              ...
              <tbody>
                {bankDetails.map(bank => (
                  <tr key={bank.id} className="border-t">
                    <td className="p-4">{bank.bank_name}</td>
                    <td className="p-4">{bank.account_name}</td>
                    <td className="p-4">{bank.account_number}</td>
                    <td className="p-4">{bank.branch || '—'}</td>
                    <td className="p-4">{bank.swift_code || '—'}</td>
                    <td className="p-4">
          ...
          <div className="bg-white rounded-xl shadow-2xl max-w-lg w-full p-6 animate-fade-in max-h-[90vh] overflow-y-auto">
            ...
            <div className="mt-6 flex gap-4">
              <button onClick={handleAssignMultiSubmit} disabled={assigning} className="bg-blue-600 text-white px-4 py-2 rounded-lg flex items-center gap-2">
                {assigning ? <FaSpinner className="animate-spin" /> : null}
                {assigning ? 'Assigning...' : 'Assign All Fees'}
              </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
          <div className="bg-white rounded-xl shadow-2xl max-w-md w-full p-6 animate-fade-in">
            ...
            <p className="text-sm text-gray-600 mb-4">
              {editingFeeForPaid.student?.first_name} {editingFeeForPaid.student?.last_name} – {editingFeeForPaid.fee_type?.name}<br />
              Total Fee: MK {Number(editingFeeForPaid.total_amount).toLocaleString()}
            </p>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium">Current Paid Amount (MK)</label>
                <input
                  type="text"
                  value={Number(editingFeeForPaid.paid_amount).toLocaleString()}
                  readOnly
                  className="w-full p-2 border rounded bg-gray-100 cursor-not-allowed"
                />
              </div>
              <div>
                <label className="block text-sm font-medium">New Paid Amount (MK) *</label>
                <input
                  type="number"
                  min="0"
                  max={editingFeeForPaid.total_amount}
                  step="0.01"
                  value={newPaidAmount}
              ...
              <button onClick={handleEditPaidAmount} disabled={savingPaidAmount} className="bg-blue-600 text-white px-4 py-2 rounded-lg w-full">
                {savingPaidAmount ? <FaSpinner className="animate-spin inline mr-1" /> : null}
                {savingPaidAmount ? 'Saving...' : 'Update Paid Amount'}
              </button>
            </div>
      ...
      {paymentModal && selectedFee && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
          <div className="bg-white rounded-xl shadow-2xl max-w-md w-full p-6 animate-fade-in">
            ...
            <p className="text-sm text-gray-600 mb-4">
              {selectedFee.student?.first_name} {selectedFee.student?.last_name} – {selectedFee.fee_type?.name}<br />
              Total: MK {Number(selectedFee.total_amount).toLocaleString()} | Paid: MK {Number(selectedFee.paid_amount).toLocaleString()}<br />
              Balance: MK {Number(selectedFee.total_amount - selectedFee.paid_amount).toLocaleString()}
            </p>
            <div className="space-y-4">
              <div>
                <label className="block text-sm">Amount *</label>
                <input type="number" min="0.01" step="0.01" max={selectedFee.total_amount - selectedFee.paid_amount} value={paymentForm.amount} onChange={e => setPaymentForm({...paymentForm, amount: e.target.value})} className="w-full p-2 border rounded" required />
              </div>
              <div>
                <label className="block text-sm">Payment Date</label>
                <input type="date" value={paymentForm.payment_date} onChange={e => setPaymentForm({...paymentForm, payment_date: e.target.value})} className="w-full p-2 border rounded" />
              </div>
              <div>
                <label className="block text-sm">Method</label>
                <select value={paymentForm.method} onChange={e => setPaymentForm({...paymentForm, method: e.target.value})} className="w-full p-2 border rounded">
                  <option>Cash</option>
              ...
              <div>
                ...
                <input type="text" value={paymentForm.notes} onChange={e => setPaymentForm({...paymentForm, notes: e.target.value})} className="w-full p-2 border rounded" />
              </div>
      ...
      {transferModal && transferSource && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
          <div className="bg-white rounded-xl shadow-2xl max-w-md w-full p-6 animate-fade-in">
            ...
            <p className="text-sm text-gray-600 mb-4">
              Source: {transferSource.student?.first_name} – {transferSource.fee_type?.name} (Overpaid: MK {Number(transferSource.paid_amount - transferSource.total_amount).toLocaleString()})
            </p>
            <div className="space-y-4">
              <div>
                <label className="block text-sm">Transfer to Fee</label>
                <select value={transferTarget} onChange={e => setTransferTarget(e.target.value)} className="w-full p-2 border rounded">
                  <option value="">Select destination</option>
                  {studentFees.filter(sf => sf.id !== transferSource.id && sf.student_id === transferSource.student_id).map(sf => (
                    <option key={sf.id} value={sf.id}>{sf.fee_type?.name} (Term: {sf.term?.name || 'N/A'})</option>
                  ))}
                </select>
              </div>
              <div>
                <label className="block text-sm">Amount</label>
                <input type="number" min="0.01" max={transferSource.paid_amount - transferSource.total_amount} value={transferAmount} onChange={e => setTransferAmount(e.target.value)} className="w-full p-2 border rounded" />
              </div>
            ...
            <form onSubmit={handleFeeTypeSubmit} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <select value={feeTypeForm.category} onChange={e => setFeeTypeForm({...feeTypeForm, category: e.target.value})} className="w-full p-2 border rounded" required>
                    {FEE_CATEGORIES.map(cat => <option key={cat} value={cat}>{cat}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-sm">Name *</label>
                  <input type="text" value={feeTypeForm.name} onChange={e => setFeeTypeForm({...feeTypeForm, name: e.target.value})} className="w-full p-2 border rounded" required />
                </div>
                <div className="md:col-span-2">
                  <label className="block text-sm">Description</label>
                  <textarea value={feeTypeForm.description} onChange={e => setFeeTypeForm({...feeTypeForm, description: e.target.value})} className="w-full p-2 border rounded" rows={2} />
                </div>
                <div className="flex items-center gap-2">
                  <input type="checkbox" checked={feeTypeForm.is_mandatory} onChange={e => setFeeTypeForm({...feeTypeForm, is_mandatory: e.target.checked})} id="mandatory" />
                  <label htmlFor="mandatory">Mandatory (auto‑assigned to students)</label>
                </div>
                <div>
                  <label className="block text-sm">Restrict to Class (optional)</label>
                  <select value={feeTypeForm.class_id} onChange={e => handleFeeTypeClassChange(e.target.value)} className="w-full p-2 border rounded">
                ...
                {feeTypeForm.class_id && availableStreams.length > 0 && (
                  <div>
                    <label className="block text-sm">Stream</label>
                    <select value={feeTypeForm.stream_id} onChange={e => setFeeTypeForm({...feeTypeForm, stream_id: e.target.value})} className="w-full p-2 border rounded">
                      <option value="">All Streams</option>
                      {availableStreams.map(s => <option key={s.id} value={s.id}>{s.stream_name || s.name}</option>)}
                    </select>
                  </div>
                ...
                {feeTypeForm.amounts.map((amt, idx) => (
                  <div key={idx} className="flex flex-wrap items-end gap-2 mb-2">
                    <div className="flex-1 min-w-[120px]">
                      ...
                      <select value={amt.class_id} onChange={e => handleAmountRowChange(idx, 'class_id', e.target.value)} className="w-full p-2 border rounded">
                    ...
                    {amt.class_id && (feeTypeStreams[idx]?.length > 0) && (
                      <div className="flex-1 min-w-[120px]">
                        ...
                        <select value={amt.stream_id} onChange={e => handleAmountRowChange(idx, 'stream_id', e.target.value)} className="w-full p-2 border rounded">
                          <option value="">All</option>
                          {feeTypeStreams[idx]?.map(s => <option key={s.id} value={s.id}>{s.stream_name || s.name}</option>)}
                        </select>
                      </div>
                    )}
                    <div className="flex-1 min-w-[120px]">
                      <label className="text-xs">Location (optional)</label>
                      <input type="text" value={amt.location} onChange={e => handleAmountRowChange(idx, 'location', e.target.value)} placeholder="e.g., Area A" className="w-full p-2 border rounded" />
                    </div>
                    <div className="flex-1 min-w-[100px]">
                      <label className="text-xs">Amount (MK) *</label>
                      <input type="number" min="0" step="0.01" value={amt.amount} onChange={e => handleAmountRowChange(idx, 'amount', e.target.value)} className="w-full p-2 border rounded" required />
                    </div>
                    {feeTypeForm.amounts.length > 1 && (
                      <button type="button" onClick={() => removeAmountRow(idx)} className="text-red-500 p-2"><FaTrash /></button>
                    )}
                  </div>
                ))}
              ...
              <div className="flex gap-4 mt-6">
                <button type="submit" disabled={savingFeeType} className="bg-blue-600 text-white px-4 py-2 rounded-lg flex items-center gap-2">
                  {savingFeeType ? <FaSpinner className="animate-spin" /> : null}
            ...
            <form onSubmit={handleBankSubmit} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" value={bankForm.bank_name} onChange={e => setBankForm({...bankForm, bank_name: e.target.value})} required className="w-full p-2 border rounded" />
                </div>
                <div>
                  <label className="block text-sm">Account Name *</label>
                  <input type="text" value={bankForm.account_name} onChange={e => setBankForm({...bankForm, account_name: e.target.value})} required className="w-full p-2 border rounded" />
                </div>
                <div>
                  <label className="block text-sm">Account Number *</label>
                  <input type="text" value={bankForm.account_number} onChange={e => setBankForm({...bankForm, account_number: e.target.value})} required className="w-full p-2 border rounded" />
                </div>
                <div>
                  <label className="block text-sm">Branch (optional)</label>
                  <input type="text" value={bankForm.branch} onChange={e => setBankForm({...bankForm, branch: e.target.value})} className="w-full p-2 border rounded" />
                </div>
                <div>
                  <label className="block text-sm">Swift Code (optional)</label>
                  <input type="text" value={bankForm.swift_code} onChange={e => setBankForm({...bankForm, swift_code: e.target.value})} className="w-full p-2 border rounded" />
                </div>
              </div>
              <div className="flex gap-4">
                <button type="submit" disabled={savingBank} className="bg-blue-600 text-white px-4 py-2 rounded-lg flex items-center gap-2">
                  {savingBank ? <FaSpinner className="animate-spin" /> : null}
